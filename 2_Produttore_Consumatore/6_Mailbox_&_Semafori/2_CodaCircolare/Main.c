...
int main() {
	...
	pid_t pid;
	key_t Chiave_MSG = ftok ("./",'C'); // chiave del messaggio
	...
	int ds_msg = msgget(Chiave_MSG, IPC_CREAT|0664);
	if(ds_msg<0){perror("MSG errore"); exit(1); }
	...
	Remove_Dati(c,ds_msg,ds_sem,ds_shm);
	
    return 0;
}

void Produttore(Struct* c ,int ds_msg, int ds_sem){
	
	srand(getpid()*time(NULL));
	for(int i=0;i<Num_Produzioni;i++){
		sleep(2);
		Produzione(c,ds_msg,ds_sem);
	}
	
}

void Consumatore(Struct* c,int ds_msg, int ds_sem){
	
	for(int i=0;i<Num_Consumazioni;i++){
		sleep(1);
		Consumazione(c,ds_msg,ds_sem);
	}	
